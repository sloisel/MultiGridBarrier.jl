...
if cuda_ok
    @testset "CUDA extension" begin
        @testset "fem1d CUDA geometric_mg vs native" begin
            ...
            sol_cuda = cuda_to_native(mgb_solve(native_to_cuda(geometric_mg(fem1d(; nodes=collect(range(-1.0, 1.0, length=9))), 3)); verbose=false))
        ...
        @testset "fem2d_P2 CUDA geometric_mg vs native" begin
            ...
            sol_cuda = cuda_to_native(mgb_solve(native_to_cuda(geometric_mg(fem2d_P2(), 3)); verbose=false))
            @test maximum(abs.(sol_native.z .- sol_cuda.z)) < 1e-8
        end
        @testset "fem3d CUDA geometric_mg vs native" begin
            sol_native = mgb_solve(geometric_mg(fem3d(; k=3), 2); verbose=false)
            sol_cuda = cuda_to_native(mgb_solve(native_to_cuda(geometric_mg(fem3d(; k=3), 2)); verbose=false))
            @test maximum(abs.(sol_native.z .- sol_cuda.z)) < 1e-8
        end
