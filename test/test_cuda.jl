...
if cuda_ok
    @testset "CUDA extension" begin
        ...
        # AMG-FEM CUDA front-ends via amg(native_to_cuda(...)). The algebraic prolongations
        # don't have regular block structure, so pass structured=false.
        ...
        @testset "fem1d CUDA AMG vs native AMG" begin
            ...
            sol_cuda = cuda_to_native(mgb_solve(native_to_cuda(amg(fem1d(; nodes=nodes5)); structured=false); p=1.0, verbose=false))
            @test maximum(abs.(sol_native.z .- sol_cuda.z)) < 1e-8
        end
        @testset "fem2d_P1 CUDA AMG vs native AMG" begin
            sol_native = mgb_solve(amg(subdivide(fem2d_P1(), 2)); p=1.0, verbose=false)
            sol_cuda = cuda_to_native(mgb_solve(native_to_cuda(amg(subdivide(fem2d_P1(), 2)); structured=false); p=1.0, verbose=false))
            @test maximum(abs.(sol_native.z .- sol_cuda.z)) < 1e-8
        end
        @testset "fem2d_P2 CUDA AMG vs native AMG" begin
            sol_native = mgb_solve(amg(subdivide(fem2d_P2(), 2)); p=1.0, verbose=false)
            sol_cuda = cuda_to_native(mgb_solve(native_to_cuda(amg(subdivide(fem2d_P2(), 2)); structured=false); p=1.0, verbose=false))
            @test maximum(abs.(sol_native.z .- sol_cuda.z)) < 1e-8
        end
        @testset "fem3d CUDA AMG vs native AMG" begin
            sol_native = mgb_solve(amg(subdivide(fem3d(; k=1), 2)); p=1.0, verbose=false)
            sol_cuda = cuda_to_native(mgb_solve(native_to_cuda(amg(subdivide(fem3d(; k=1), 2)); structured=false); p=1.0, verbose=false))
            @test maximum(abs.(sol_native.z .- sol_cuda.z)) < 1e-8
        end
    end
else
