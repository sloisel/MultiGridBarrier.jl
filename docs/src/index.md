...
using PyPlot # hide
using MultiGridBarrier
fem_solve1d(L=5,p=1.0,verbose=false);
...
```@example 1
fem_solve2d(L=3,p=1.0,verbose=false);
...
```@example 1
spectral_solve1d(n=40,p=1.0,verbose=false);
...
A 2d p-Laplace problem:
```@example 1
spectral_solve2d(n=5,p=1.5,verbose=false);
