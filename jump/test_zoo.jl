# Cross-validation of the JuMP modeling layer against the classical API.
#
# All six Zoo problems (2D, default parameters) are rebuilt through the JuMP
# front end and compared component-wise against the classical constructors
# `mgb_solve(Zoo.<problem>(amg(geom)))` on the same geometry. The discrete
# problems are identical up to D-row / barrier-piece ordering, so solutions
# must agree to solver tolerance (empirically: bit-identical, or one ulp).
# A final testset exercises region-restricted cones (`On`), `convex_piecewise`
# lowering, and the feasibility phase.
#
# Runs two ways:
#   - standalone:  julia --project=<env with MultiGridBarrier + JuMP> test_zoo.jl
#   - in the package test suite, via include from test/test_jump.jl
#     (JuMP is a test-only dependency; the package itself does not depend on it).
...
@testset "JuMP front end" begin
    ...
    @testset "minimal_surface" begin
        ...
        println("minimal_surface:       max diff = ", d)
        @test d < _JUMP_MATCH_TOL
    end
    ...
    @testset "norton_hoff" begin
        ...
        println("norton_hoff:           max diff = ", d)
        @test d < _JUMP_MATCH_TOL
    end
    ...
    @testset "two_sided_obstacle" begin
        ...
        println("two_sided_obstacle:    max diff = ", d)
        @test d < _JUMP_MATCH_TOL
    end
    ...
    @testset "elastoplastic_torsion" begin
        ...
        println("elastoplastic_torsion: max diff = ", d)
        @test d < _JUMP_MATCH_TOL
    end

    @testset "region-restricted cone (On) + feasibility phase" begin
        # Obstacle imposed only on the left half: exercises the
        # convex_piecewise selector lowering and, via the infeasible start,
        # the feasibility phase. The obstacle must bind on the region and be
        # genuinely absent off it.
