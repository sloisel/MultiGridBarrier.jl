export geometric_fem1d, FEM1D, geometric_fem1d_solve
...
"""
    geometric_fem1d_solve(::Type{T}=Float64; kwargs...) -> AMGBSOL

Solve a 1D FEM problem. Keyword arguments are passed to both `geometric_fem1d` (e.g. `L`)
and `amgb` (e.g. `p`, `verbose`). See `amgb` for the full list.
"""
geometric_fem1d_solve(::Type{T}=Float64;rest...) where {T} = amgb(geometric_fem1d(T;rest...);rest...)

"""
    geometric_fem1d(::Type{T}=Float64; L=4, kwargs...)
...
"""
function geometric_fem1d(::Type{T}=Float64;L::Int=4,structured::Bool=true,rest...) where {T}
