"""
    algebraic_fem1d(::Type{T}=Float64; nodes, max_coarse=2) -> Geometry
...
- `max_coarse`: AMG keeps coarsening until the coarsest level has at most
  ...
  a single global mode — matching `MultiGridBarrier.fem1d`'s geometric
  depth and what `amgb`'s slash cycle expects.

# Example
```julia
geom = algebraic_fem1d(; nodes = collect(range(-1.0, 1.0, length=33)))
sol  = amgb(geom; p=1.5)
```
...
"""
function algebraic_fem1d(::Type{T}=Float64; nodes::AbstractVector,
...
"""
    algebraic_fem1d_solve(::Type{T}=Float64; rest...) -> AMGBSOL

Solve a 1D Dirichlet variational problem with P1 elements on the mesh you
supply. Equivalent to `amgb(algebraic_fem1d(T; rest...); rest...)`: keyword
arguments are forwarded to both `algebraic_fem1d` (mesh kwargs `nodes`,
`max_coarse`) and `amgb` (solver kwargs `p`, `f`, `g`, `verbose`, …).

# Example
```julia
sol = algebraic_fem1d_solve(nodes = collect(range(-1.0, 1.0, length=33)), p = 1.5)
```

See `amgb` for the full set of solver kwargs.
"""
algebraic_fem1d_solve(::Type{T}=Float64; rest...) where {T} =
    amgb(algebraic_fem1d(T; rest...); rest...)
...
# 2 n_e × (n_e - 1): fem1d's Dirichlet subspace on the doubled basis.
